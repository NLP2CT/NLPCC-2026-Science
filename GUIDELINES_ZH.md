...
证据材料中 `img_path` 所引用的图像文件单独提供在 `images.zip` 中。
...
**第一阶段 — 公开评测（2026 年 5 月 25 日 -- 6 月 20 日）：**
在报名截止日（5 月 25 日）前，我们将发布公开测试数据、在线提交链接（暂定为 Kaggle）以及离线评测工具。参赛队伍须按官方格式将结果文件提交至平台以获得第一阶段分数。公开排行榜将持续开放至最终提交截止日。
...
1. **结果文件**：按官方提交格式，将第一阶段（公开）和第二阶段（隐藏）的预测结果提交至平台。
...
| 2026 年 5 月 25 日 | 报名截止；发布公开测试数据与公开排行榜**（第一阶段开始）** |
